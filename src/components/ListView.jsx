...
export const CATEGORIES = ['Travel', 'Adventure', 'Food & Drink', 'Home', 'Milestones', 'Movies', 'Other']
...
export default function ListView({ listId, name, onSwitch }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => {
        ...
        setLoadError(error.message || 'Something went wrong loading this list.')
        setLoading(false)
      }
    )
    return () => unsub()
  ...
  const filtered = useMemo(() => {
    ...
    else if (sortBy === 'oldest') sorted.sort((a, b) => getTime(a) - getTime(b))
  ...
  async function handleAdd(data) {
    ...
    // Form stays open and clears itself (see AddItemForm) so you can
    // add several items in a row. Closing it is now a manual action via Cancel.
  }
      ...
      <header className="max-w-4xl mx-auto px-6 pt-12 pb-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h1 className="font-display text-4xl text-parchment font-semibold">Our Bucket List</h1>
          </div>
          <button
            onClick={onSwitch}
            className="text-muted text-xs uppercase tracking-wide hover:text-parchment border border-surfacealt rounded-full px-4 py-2 mt-2"
          >
            Switch list
          </button>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        {!loading && loadError && (
          <div className="text-center py-16 border border-rose/40 rounded-2xl bg-rose/5">
            <p className="font-display text-2xl text-parchment mb-2">Couldn't load this list.</p>
        ...
        {!loading && !loadError && filtered.length === 0 && (
          <div className="text-center py-16 border border-dashed border-surfacealt rounded-2xl">
            <p className="font-display text-2xl text-parchment mb-2">A blank page.</p>
